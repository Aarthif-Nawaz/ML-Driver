...
function CreatePlanner () {
    ...
    const saveData = async (e) => {
        ...
        if (!fname.trim().length || !lname.trim().length || !age.trim().length || !gender.trim().length || !address.trim().length || !email.trim().length || !password.trim().length){
            ...
            console.log(fname)
            const data = await signup({fname,lname,age,gender,address,email,password, type: 'planner'})
            console.log(data)
            toast.success(`Saved Created Planner ${fname + " " + lname}`, {
                    ...
                    <div className="col-lg-12 col-md-12">
                        <div className="card">
                            <div className="body">
                                ...
                                <span onClick={saveData} style={{ marginTop: 20 }} className="btn btn-sm btn-primary mr-1" title="">Create Planner</span>
                            </div>
                        </div>
                    </div>
                </div>
