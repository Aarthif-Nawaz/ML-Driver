			...
			<>
				<div id="left-sidebar" className={`sidebar${this.props.miniSideMenuOn ? ' mini_sidebar_on' : ''}`}>
					<div className="navbar-brand">
						<Link to="/"><img src="../assets/images/icon.svg" alt="Oculux Logo" className="img-fluid logo" /><span>Booking System</span></Link>
					...
					<div className="sidebar-scroll">
							...
							<div className="dropdown">
								...
								<Dropdown className="ml-0">
									<Dropdown.Toggle variant="success" id="dropdown-basic" className="user-name left_dropdown_btn">
										<strong>{localStorage.getItem('type') === "admin" ? "Admin" : localStorage.getItem('email')}</strong>
									</Dropdown.Toggle>

									<Dropdown.Menu>
										{/* <Link className="dropdown-item" to="/page-profile"><i className="icon-user"></i>My Profile</Link> */}
										<Link className="dropdown-item" to="/"><i className="icon-power"></i>Logout</Link>
									</Dropdown.Menu>
								</Dropdown>
						...
						<nav id="left-sidebar-nav" className="sidebar-nav">
							<MetisMenu
								content={localStorage.getItem('type')!=="admin" ? metisMenu : metisMenu2}
